defmodule ParserTest do
  ...
  test "Parse all available blocks" do
    {:ok, blocks, rest} = ParseBlockChain.parse_available_blocks(@bootstrap_start_snippet)
    assert length(blocks) == 2
    assert byte_size(rest) > 0
  end

  test "Parse all available blocks with rest data" do
    <<data1 :: binary-size(400), data2 :: binary>> = @bootstrap_start_snippet
    {:ok, blocks1, rest} = ParseBlockChain.parse_available_blocks(data1)
    assert byte_size(rest) > 0
    {:ok, blocks2, rest} = ParseBlockChain.parse_available_blocks(<<rest :: binary, data2 :: binary>>)
    assert length(blocks1 ++ blocks2) == 2
    assert byte_size(rest) > 0
  end
  ...
  test "Get raw block from chain" do
    # Get first block ok.
    {:ok, _block, rest} = Parser.get_raw_block(@bootstrap_start_snippet)
